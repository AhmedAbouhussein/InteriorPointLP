function [x,y,s,Z,W,k,T,G,R,M] = PDM_f(A,b,c,x0,y0,s0,mu0,theta,tol,N)
% Uses the Feasible Primal-Dual Method to solve
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if G(1) > tol                   % If duality gap is large enough
      for k = 1:N                     % Then execute the method
        ...
        dy = -(A*D*A')\(A*Si*v);  % Compute Direction of y
        ds = -A'*dy;              % Compute Direction of s
